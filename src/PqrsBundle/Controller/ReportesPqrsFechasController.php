...
class ReportesPqrsFechasController extends Controller {
            ...
            if (in_array(1, $areas) || in_array('ADMIN', $tokens['roles'])):                
                ...
                if ($form->isValid()):
                    if (strlen($listado_id_multiplex) == 0):
                        ...
                        $dql = "select p from PqrsBundle:Pqrs p
                            WHERE p.fecha_modificacion BETWEEN " . $fecha_ini . " AND " . $fecha_fin . "
                            AND 
                ...
                foreach ($pqrs as $pq):
                    ...
                    if (is_array($respuestas_pqrs) && !empty($respuestas_pqrs)):
                        $ultima_respuesta = $respuestas_pqrs[0]->getRespuesta();
                        foreach ($respuestas_pqrs[0]->getCausa() as $causas):
                            $nombres_causas .= $causas->getCausa() . ',';
                        endforeach;
                    endif;
                    $nombres_causas = substr($nombres_causas, 0, -1);
                    $datos_pqrs[$pq->getId()]['respuesta'] = $ultima_respuesta;
                    $datos_pqrs[$pq->getId()]['causas'] = $nombres_causas;
                ...
                return $this->render('PqrsBundle:Reportes:pqrs_fechas.html.twig', array('form' => $form->createView(), 'pqrs' => $pqrs, 'datos' => $datos_pqrs, 'filtros' => $filtros, 'permisos' => $permisos, 'areas' => $areas, 'roles' => $tokens['roles'],));
            else:
                /*$this->get('session')->getFlashBag()->add(
    ...
    public function generarAction($filtros) {
        ...
        if ($datos_filtro['multiplex']):
            $em = $this->getDoctrine()->getManager();
            $dql = "select p from PqrsBundle:Pqrs p
                        WHERE p.fecha_modificacion BETWEEN " . $datos_filtro['fecha_ini'] . " AND " . $datos_filtro['fecha_fin'] . "
                        AND 
            ...
            $phpExcelObject->getActiveSheet()->getStyle('A7:O7')->applyFromArray($styleTitulos);
            $phpExcelObject->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('M7', 'Última Respuesta')
                    ->setCellValue('N7', 'Causa Final')
                    ->setCellValue('O7', 'Teléfono');
            $i = 8;
            $j = 1;
            foreach ($pqrs as $pq):
                $nombres_areas = '';
                foreach ($datos_pqrs[$pq->getId()]['nombres_areas'] as $areas):
                    $nombres_areas .= $areas . ',';
                endforeach;
                $nombres_areas = substr($nombres_areas, 0, -1);
                $phpExcelObject->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('M' . $i, $datos_pqrs[$pq->getId()]['respuesta'])
                        ->setCellValue('N' . $i, (string)$datos_pqrs[$pq->getId()]['causas'])
                        ->setCellValue('O' . $i, $pq->getTelefono());
                $phpExcelObject->getActiveSheet()->getStyle("A8:O$i")->applyFromArray($styleResultados);
                $i++;
                $j++;
            endforeach;
